#!/usr/bin/env bash
# Wrapper for playbooks/create-vm-centos.yml: spins up a local CentOS Stream 10
# GenericCloud libvirt VM. This is the CentOS Stream counterpart of
# create-vm.sh (which targets Fedora Cloud); keep the two in sync. See
# playbooks/README.md for details.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PLAYBOOK="${SCRIPT_DIR}/../playbooks/create-vm-centos.yml"

usage() {
  cat <<'EOF'
Usage: create-vm-centos.sh <vm_name> [-c vcpus] [-m memory_mb] [-d disk_gb] [-n network]
...
Example:
  create-vm-centos.sh my-centos-vm
EOF
}
