#!/usr/bin/env bash
# Wrapper for playbooks/provision-vm-centos.yml: provisions a CentOS Stream 10
# GenericCloud VM for rootless Podman. This is the CentOS Stream counterpart of
# provision-vm.sh (which targets Fedora Cloud) — the two are identical except
# for the playbook they invoke and the default VM name, so keep them in sync.
#
# See playbooks/README.md for bootstrap requirements — the first run must
# connect as root (-u root); subsequent runs should use -u zedr.
#
...
PLAYBOOK="${SCRIPT_DIR}/../playbooks/provision-vm-centos.yml"
...
usage() {
  cat <<'EOF'
Usage: provision-vm-centos.sh [vm_name] [-H host] [-u ssh_user] [-k ssh_pubkey_file] [-- ansible-playbook-args...]

  [vm_name]           libvirt domain name to resolve via `virsh domifaddr`
                       (default: centos-stream-10). Ignored if -H is given.
...
Examples:
  ...
  provision-vm-centos.sh -u root

  # Target a specific libvirt VM by name
  provision-vm-centos.sh my-centos-vm -u root

  # Target a host directly, skipping the virsh lookup
  provision-vm-centos.sh -H 192.168.122.50 -u zedr
EOF
}
...
# provision-vm-centos.yml targets the `mojesaldoo_vms` group, so build a
# throwaway ini inventory for the single target host rather than requiring a
